var lastUpdated = "2026-09-19 00:15";

var officialConfig = [
  {
    "name": "乃木坂46 官方网站",
    "link": "https://www.nogizaka46.com",
    "group": "nogi"
  },
  {
    "name": "乃木坂46 新闻",
    "link": "https://www.nogizaka46.com/s/n46/news/list",
    "group": "nogi"
  },
  {
    "name": "乃木坂46 日程",
    "link": "https://www.nogizaka46.com/s/n46/media/list",
    "group": "nogi"
  },
  {
    "name": "乃木坂46 成员",
    "link": "https://www.nogizaka46.com/s/n46/search/artist",
    "group": "nogi"
  },
  {
    "name": "乃木坂46 博客",
    "link": "https://www.nogizaka46.com/s/n46/diary/MEMBER",
    "group": "nogi"
  },
  {
    "name": "OFFICIAL WEB SHOP",
    "link": "https://www.nogizaka46shop.com/",
    "group": "nogi"
  },
  {
    "name": "乃木坂46 Mobile",
    "link": "https://sp.nogizaka46.com",
    "group": "nogi"
  },
  {
    "name": "乃木动画",
    "link": "https://nogidoga.com/",
    "group": "nogi"
  },
  {
    "name": "MYNOGI",
    "link": "https://mynogi.snft.jp/",
    "group": "nogi"
  },
  {
    "name": "线握/CD购入活动日程",
    "link": "https://www.nogizaka46.com/s/n46/live?ima=0000&ct=HANDSHAKE",
    "group": "nogi"
  },
  {
    "name": "乃木坂46 Message",
    "link": "https://message.nogizaka46.com/",
    "group": "nogi"
  },
  {
    "name": "乃木坂工事中",
    "link": "https://tv-aichi.co.jp/nogi-kou/",
    "group": "nogi"
  },
  {
    "name": "櫻坂46 官方网站",
    "link": "https://sakurazaka46.com/s/s46/",
    "group": "saku"
  },
  {
    "name": "櫻坂46 新闻",
    "link": "https://sakurazaka46.com/s/s46/news/list",
    "group": "saku"
  },
  {
    "name": "櫻坂46 日程",
    "link": "https://sakurazaka46.com/s/s46/media/list",
    "group": "saku"
  },
  {
    "name": "櫻坂46 成员",
    "link": "https://sakurazaka46.com/s/s46/search/artist",
    "group": "saku"
  },
  {
    "name": "櫻坂46 博客",
    "link": "https://sakurazaka46.com/s/s46/diary/blog/list",
    "group": "saku"
  },
  {
    "name": "OFFICIAL GOODS STORE",
    "link": "https://store.plusmember.jp/sakurazaka46/",
    "group": "saku"
  },
  {
    "name": "MY PAGE",
    "link": "https://sakurazaka46.com/s/s46/mypage",
    "group": "saku"
  },
  {
    "name": "经纪人日记",
    "link": "https://sakurazaka46.com/s/s46/diary/managers_diary/list",
    "group": "saku"
  },
  {
    "name": "樱耳广播",
    "link": "https://sakurazaka46.com/s/s46/diary/radio",
    "group": "saku"
  },
  {
    "name": "见面会日程",
    "link": "https://sakurazaka46.com/s/s46/diary/event_page/list?ima=0000",
    "group": "saku"
  },
  {
    "name": "櫻坂46 Message",
    "link": "https://message.sakurazaka46.com/",
    "group": "saku"
  },
  {
    "name": "转角就是樱坂？",
    "link": "https://www.tv-tokyo.co.jp/keyaki/",
    "group": "saku"
  },
  {
    "name": "日向坂46 官方网站",
    "link": "https://www.hinatazaka46.com/s/official/",
    "group": "hina"
  },
  {
    "name": "日向坂46 新闻",
    "link": "https://www.hinatazaka46.com/s/official/news/list",
    "group": "hina"
  },
  {
    "name": "日向坂46 日程",
    "link": "https://www.hinatazaka46.com/s/official/media/list",
    "group": "hina"
  },
  {
    "name": "日向坂46 成员",
    "link": "https://www.hinatazaka46.com/s/official/search/artist",
    "group": "hina"
  },
  {
    "name": "日向坂46 博客",
    "link": "https://www.hinatazaka46.com/s/official/diary/member",
    "group": "hina"
  },
  {
    "name": "OFFICIAL GOODS STORE",
    "link": "https://store.plusmember.jp/hinatazaka46/",
    "group": "hina"
  },
  {
    "name": "我的页面",
    "link": "https://www.hinatazaka46.com/s/official/mypage",
    "group": "hina"
  },
  {
    "name": "经纪人日记",
    "link": "https://www.hinatazaka46.com/s/official/diary/manager/list",
    "group": "hina"
  },
  {
    "name": "日向坂46 fanclub",
    "link": "https://www.hinatazaka46.com/s/official/page/about_fanclub",
    "group": "hina"
  },
  {
    "name": "见面会日程",
    "link": "https://www.hinatazaka46.com/s/official/diary/event/list?ima=0000",
    "group": "hina"
  },
  {
    "name": "日向坂46 Message",
    "link": "https://message.hinatazaka46.com/",
    "group": "hina"
  },
  {
    "name": "在日向坂见面吧",
    "link": "https://www.tv-tokyo.co.jp/hinatazaka/",
    "group": "hina"
  }
];

var toolConfig = [
  {
    "name": "⭐乃木坂46 SNS",
    "desc": "Telegram 频道",
    "detail": "・[乃木坂46 OFFICIAL YouTube CHANNEL](https://www.youtube.com/@nogizaka46SMEJ)\n・[乃木坂配信中](https://www.youtube.com/@nogizakahaishinchu)\n・[X（旧Twitter）](https://x.com/nogizaka46)\n・[Instagram](https://www.instagram.com/nogizaka46_official/)\n・[Facebook](https://www.facebook.com/nogizaka46)\n・[TikTok](https://www.tiktok.com/@nogizaka46_official)\n・[Line](https://lin.ee/EwCV3ub)\n・[Weibo](https://weibo.com/u/6372196907)\n・[Bilibili](https://space.bilibili.com/603028270)",
    "tags": [
      "公式",
      "sns",
      "blog",
      "information",
      "photo"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://t.me/s/nogizaka46_SNS/"
  },
  {
    "name": "⭐櫻坂46 SNS",
    "desc": "Telegram 频道",
    "detail": "・[櫻坂46 OFFICIAL YouTube CHANNEL](https://www.youtube.com/@sakurazaka46SMEJ)\n・[樱坂チャンネル](https://www.youtube.com/@sakurazakachannel/)\n・[X（旧Twitter）](https://x.com/sakurazaka46)\n・[Instagram @sakurazaka46jp](https://www.instagram.com/sakurazaka46jp)\n・[Instagram @sakurazaka46_info_official](https://www.instagram.com/sakurazaka46_info_official/)\n・[TikTok](https://www.tiktok.com/@sakurazaka46.officialtk)",
    "tags": [
      "公式",
      "sns",
      "blog",
      "information",
      "photo"
    ],
    "groups": [
      "saku"
    ],
    "link": "https://t.me/s/sakurazaka46_sns/"
  },
  {
    "name": "⭐日向坂46 SNS",
    "desc": "Telegram 频道",
    "detail": "・[日向坂46 OFFICIAL YouTube CHANNEL](https://www.youtube.com/@46officialyoutubechannel99)\n・[日向坂ちゃんねる](https://www.youtube.com/@hinatazakachannel)\n・[X（旧Twitter）](https://x.com/hinatazaka46)\n・[Instagram](https://www.instagram.com/hinatazaka46)\n・[TikTok](https://www.tiktok.com/@hinatazakanews)",
    "tags": [
      "公式",
      "sns",
      "blog",
      "information",
      "photo"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://t.me/s/hinatazaka46_sns/"
  },
  {
    "name": "⭐坂道46LOVER",
    "desc": "nogi46p.com",
    "detail": "坂道最新情报站\n・登场・出演情报\n・网路新闻・官方新闻\n\n・[坂道46LOVER ◢⁴⁶ / X](https://x.com/nogi46press)\n・RSSLink：[https://nogi46p.com/feed/rdf](https://nogi46p.com/feed)",
    "tags": [
      "公式",
      "information",
      "forum"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://nogi46p.com/"
  },
  {
    "name": "⭐坂道ちゃんねる",
    "desc": "sakamichi.f-frontier.com",
    "detail": "坂道相关新闻情报站\n・官方博客\n・每日日程汇总\n・网路新闻・官方新闻\n\n・[◢ 坂道推し / X](https://x.com/sakamichi__oshi)\n・RSSLink：[https://sakamichi.f-frontier.com/feed/rdf](https://sakamichi.f-frontier.com/feed)",
    "tags": [
      "公式",
      "information",
      "forum"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://sakamichi.f-frontier.com/"
  },
  {
    "name": "⭐乃木坂46データベース",
    "desc": "n46db.com",
    "detail": "坂道成员个人资料与参加歌曲、歌曲站位、作曲家・编曲家、MV与个人PV导演等情报资讯汇总。\n\n・[About this website](https://n46db.com/about.php)",
    "tags": [
      "information",
      "song",
      "photo",
      "create"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://n46db.com/"
  },
  {
    "name": "坂道雑文帐",
    "desc": "meaning-of-goodbye.com",
    "detail": "个人经营的坂道系博客。\n歌曲站位与演唱会歌单等，汇总系文章。\n相关工具：[坂道シリーズ 统合データベース検索](https://meaning-of-goodbye.com/sakamichi-search/)\n\n・[关于此网站](https://meaning-of-goodbye.com/about/)",
    "tags": [
      "song",
      "forum",
      "live"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://meaning-of-goodbye.com/"
  },
  {
    "name": "⭐坂道 统合データベース検索",
    "desc": "[坂道雑文帐] Sakamichi Universe",
    "detail": "「坂道雑文帐」运营的坂道系情报检索系统。 \n成员、歌曲、演唱会、站位、各种资讯等，以及最新活动与发售情报皆可搜索。 \n\n・[乃木坂46 演唱会表演过的歌曲总表](https://docs.google.com/spreadsheets/d/1Kxr5ScgjxmGPlP-UohWkQ47sCKxAM5rm-z_aAA4Uerk/edit?gid=0#gid=0)\n・[网站介绍文章](https://note.com/nogikeyaksh/n/n95f652f7d114)",
    "tags": [
      "information",
      "song",
      "live"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://meaning-of-goodbye.com/sakamichi-search/"
  },
  {
    "name": "坂道データベース",
    "desc": "sakamichidatabase.penguinelegy.com",
    "detail": "・最新日程\n・个人与团体汇总资料\n・歌曲资料",
    "tags": [
      "sns",
      "information",
      "song",
      "live",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://sakamichidatabase.penguinelegy.com/"
  },
  {
    "name": "⭐あの坂道を登れ",
    "desc": "anosaka.com",
    "detail": "・演唱会与活动参加REPO\n・成员出演媒体感想\n・人气排行\n・游戏攻略\n\n・[あのさか / X](https://x.com/anosaka_46)：相关资料收集・分析\n・RSSLink：[https://anosaka.com/feed](https://anosaka.com/feed)\n・[ABOUT](https://anosaka.com/about-us)",
    "tags": [
      "meet",
      "information",
      "song",
      "sale",
      "create",
      "forum",
      "live",
      "game",
      "ranking",
      "sns",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://anosaka.com/"
  },
  {
    "name": "⭐坂道データベース",
    "desc": "[あの坂道] sakamichidb.anosaka.com",
    "detail": "「あの坂道」运营的坂道系MV播放量统计非官方网站。 \n\n・[关于此网站](https://sakamichidb.anosaka.com/other/about)",
    "tags": [
      "information",
      "song",
      "streaming",
      "create",
      "tool",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://sakamichidb.anosaka.com/"
  },
  {
    "name": "⭐坂道シリーズ楽曲DB",
    "desc": "saka46.fun",
    "detail": "坂道系歌曲资料汇总网站（作曲家、MV导演、舞蹈老师等）。 ",
    "tags": [
      "song",
      "streaming",
      "live",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://saka46.fun/"
  },
  {
    "name": "乃木坂46まとめ",
    "desc": "nogizaka46.u-ff.com",
    "detail": "[乃木坂46データベース](https://nogizaka46.u-ff.com/database/)\n・最新新闻\n・成员与团体资料汇总\n・活动歌单\n・见面会资料\n・村外影片汇总",
    "tags": [
      "clothes",
      "meet",
      "information",
      "song",
      "sale",
      "forum",
      "live",
      "ranking"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://nogizaka46.u-ff.com/"
  },
  {
    "name": "日向坂46まとめ",
    "desc": "hinatazaka46.u-ff.com",
    "detail": "[日向坂46データベース](https://hinatazaka46.u-ff.com/database/)\n・最新新闻\n・成员与团体资料汇总\n・活动歌单\n・见面会资料\n・影片连结（Smartnews）\n・Podcast（Audee）",
    "tags": [
      "clothes",
      "meet",
      "information",
      "song",
      "sale",
      "streaming",
      "forum",
      "live",
      "ranking"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://hinatazaka46.u-ff.com/"
  },
  {
    "name": "まいにち日向坂",
    "desc": "hinatafan.com",
    "detail": "・演唱会歌单\n・日向坂46圣地资料\n・成员资料汇总",
    "tags": [
      "place",
      "meet",
      "information",
      "song",
      "forum",
      "live",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://hinatafan.com/"
  },
  {
    "name": "坂道46データベース",
    "desc": "zaka46db.com",
    "detail": "・团体资料\n・个人情报\n・写真集资料\n・歌曲资料\n・学力测验资料",
    "tags": [
      "sns",
      "information",
      "song"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://zaka46db.com/"
  },
  {
    "name": "新时代レポ",
    "desc": "report-newage.com",
    "detail": "・[乃木坂46 歌单](https://report-newage.com/?s=%E4%B9%83%E6%9C%A8%E5%9D%8246)\n・[櫻坂46 歌单](https://report-newage.com/?s=%E6%AB%BB%E5%9D%8246)\n・[日向坂46 歌单](https://report-newage.com/?s=%E6%97%A5%E5%90%91%E5%9D%8246)",
    "tags": [
      "live"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://report-newage.com/"
  },
  {
    "name": "SAKURA DATA 46",
    "desc": "sakurazaka46.live",
    "detail": "成员个资与昵称、生日、血型、身长等详细情报。也把歌曲资料（单曲・专辑・原表演成员・Center次数・作词作曲者）整理的更加直观。\n最新单曲情报与毕业成员的资料也正在更新中。",
    "tags": [
      "information",
      "song"
    ],
    "groups": [
      "saku"
    ],
    "link": "https://sakurazaka46.live/"
  },
  {
    "name": "HINABASE",
    "desc": "kasumizaka46.com",
    "detail": "成员个资与昵称、生日、血型、身长等详细情报。也把歌曲资料（单曲・专辑・原表演成员・Center次数・作词作曲者）整理的更加直观。\n最新单曲情报与毕业成员的资料也正在更新中。",
    "tags": [
      "information",
      "song"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://kasumizaka46.com/"
  },
  {
    "name": "坂道系直播时间序资料库",
    "desc": "wikihouse.com/sakamichi",
    "detail": "坂道系的Showroom、SNS等直播影片线上观看统整网站。",
    "tags": [
      "information",
      "streaming",
      "sns"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://www.wikihouse.com/sakamichi/index.php?FrontPage"
  },
  {
    "name": "ファン向けの応援ブログ「がなぷら」",
    "desc": "hiragana-plan.com",
    "detail": "【坂道/符号/PassCode/MFS】的个人经营博客「がなぷら」。个人资料与排行等资料汇总。 ",
    "tags": [
      "information",
      "forum",
      "ranking",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://hiragana-plan.com/"
  },
  {
    "name": "偶像宠物名汇总",
    "desc": "seesaawiki.jp/nameofidolsdog",
    "detail": "资料不完全、不确定是否还在更新",
    "tags": [
      "information"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://seesaawiki.jp/nameofidolsdog/"
  },
  {
    "name": "日向坂46　演唱会情报",
    "desc": "w.atwiki.jp/hinatazaka46liveinfo",
    "detail": "粉丝建立的日向坂46演唱会相关问题与资讯的WIKI网站。 ",
    "tags": [
      "place",
      "sns",
      "information",
      "song",
      "live",
      "oshi"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://w.atwiki.jp/hinatazaka46liveinfo/"
  },
  {
    "name": "乃木坂パープルハート",
    "desc": "nogizaka.info",
    "detail": "乃木坂46新粉丝向资讯站，内涵乃木坂46过往的小知识与杂学。 ",
    "tags": [
      "clothes",
      "information",
      "song",
      "oshi"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://www.nogizaka.info/index.html"
  },
  {
    "name": "ZERO スケジュールまとめ",
    "desc": "x.com/zero00zero46",
    "detail": "制作未来的日程总览（图片形式）的Ｘ大佬",
    "tags": [
      "公式",
      "information",
      "oshi"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://x.com/zero00zero46"
  },
  {
    "name": "⭐ 46 NEWS !",
    "desc": "46news.net",
    "detail": "日本论坛文章汇总网站。 \n来源 talk.jp->各大まとめ网站->汇总网。 \n\n以下为我自行制作的RSS，更新频率为0~30分不等。 \nRSSLink politepol.com\n・乃木坂46 [/fd/wDNr0GJnT4W6.xml](https://politepol.com/fd/wDNr0GJnT4W6.xml)\n・櫻坂46　 [/fd/tQUjLrmQlSYk.xml](https://politepol.com/fd/tQUjLrmQlSYk.xml)\n・日向坂46 [/fd/1ROHnysso4vs.xml](https://politepol.com/fd/1ROHnysso4vs.xml)",
    "tags": [
      "forum"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://46news.net/"
  },
  {
    "name": "⭐SHOWROOMデータ",
    "desc": "aknog.github.io/srdata",
    "detail": "坂道系SHOWROOM直播数据。 \n\n・[SHOWROOMデータ / X](https://x.com/srdxsa)\n・Smartnews影片连结（放课后谈话等）",
    "tags": [
      "streaming",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://aknog.github.io/srdata/"
  },
  {
    "name": "⭐アイドル雑誌情報Z",
    "desc": "idolz.hubxhub.com",
    "link": "https://idolz.hubxhub.com/",
    "detail": "女性偶像、平面偶像及女演员登场的杂志，以及相关写真集的资讯。\n\n ・[アイドル雑誌情報Z / X](https://x.com/idolzj)",
    "tags": [
      "公式",
      "photo"
    ],
    "groups": []
  },
  {
    "name": "杂志封面情报（偶像）",
    "desc": "x.com/otdmqivibpkq3so",
    "detail": "",
    "tags": [
      "公式",
      "photo"
    ],
    "groups": [],
    "link": "https://x.com/otdmqivibpkq3so"
  },
  {
    "name": "⭐坂道 store",
    "desc": "sakamichi-store.net",
    "detail": "介绍坂道成员私服・鞋子・相机・化妆品的网站。来源通常是blog/Youtube /TV/TikTok的情报🌷少量圣地巡礼情报​​。 ※只由个人特定所以有可能有错误还请见谅。 \n\n・[坂道store / X](https://x.com/sakamichi_store)",
    "tags": [
      "item",
      "clothes",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://sakamichi-store.net/"
  },
  {
    "name": "⭐N46 fashion│私服･衣装",
    "desc": "instagram.com/ngz46.fashionsnap",
    "detail": "・私服\n・衣装\n・私物\n・单品\n・爱用品",
    "tags": [
      "item",
      "clothes",
      "oshi"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://www.instagram.com/ngz46.fashionsnap/"
  },
  {
    "name": "⭐pyon アイドル私服衣装特定",
    "desc": "x.com/hntzk_46pyo",
    "detail": "偶像私服 / 衣装 / 化妆品 / 店家 etc 特定汇总推文🐻❣️",
    "tags": [
      "item",
      "clothes"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/hntzk_46pyo"
  },
  {
    "name": "⭐ひまり│坂道コスメ",
    "desc": "x.com/himari_cosme_46",
    "detail": "坂道系(主乃木坂)的爱用化妆品、护肤品、护发品汇总 ˗ˋˏ ♡ ˎˊ˗ 有时有48、符号系、早安、iLiFE!的资讯 也会发之前的资讯 如有错误请见谅",
    "tags": [
      "item",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/himari_cosme_46"
  },
  {
    "name": "ありんこ",
    "desc": "x.com/Arinko__illust",
    "link": "https://x.com/Arinko__illust",
    "detail": "・服装图鉴\n・谷子图鉴",
    "tags": [
      "clothes"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ]
  },
  {
    "name": "アイドルバケット",
    "desc": "idolbucket.jp",
    "detail": "・私服・衣装特定\n・演唱会歌单\n・应援手灯颜色",
    "tags": [
      "item",
      "clothes",
      "information",
      "song",
      "live",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://idolbucket.jp/"
  },
  {
    "name": "ゆにめいと Yunimate",
    "desc": "yunimate.jp",
    "detail": "坂道（乃木坂46・櫻坂46・欅坂46・日向坂46）衣服情报",
    "tags": [
      "clothes",
      "song"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://yunimate.jp/"
  },
  {
    "name": "乃木坂46 N46ネットワーク",
    "desc": "nogizaka46.net",
    "detail": "・乃木坂46 演唱会歌单\n・歌曲情报\n・[RSS link](https://nogizaka46.net/feed/atom)",
    "tags": [
      "clothes",
      "information",
      "song",
      "forum",
      "live"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://nogizaka46.net/"
  },
  {
    "name": "JOYFUL LOVE 虹色作戦まとめwiki",
    "desc": "w.atwiki.jp/joyfullove",
    "detail": "日向坂46的JOYFUL LOVE 虹色作战资讯汇总wiki。",
    "tags": [
      "live",
      "oshi"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://w.atwiki.jp/joyfullove/"
  },
  {
    "name": "乃木坂コール中",
    "desc": "nogizaka46-live.hatenablog.jp",
    "detail": "乃木坂46 歌曲打call资料站",
    "tags": [
      "song",
      "live",
      "oshi"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://nogizaka46-live.hatenablog.jp/"
  },
  {
    "name": "乃木坂绍介中",
    "desc": "n46hako.com",
    "detail": "基本上只有乃木坂46 的演唱会歌单",
    "tags": [
      "song",
      "live"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://n46hako.com/"
  },
  {
    "name": "SEKAIWEEK 坂道シリーズ",
    "desc": "the0ries.com",
    "detail": "坂道系资讯汇总文章",
    "tags": [
      "information",
      "song",
      "sale",
      "forum"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://the0ries.com/category/nogizaka/"
  },
  {
    "name": "坂道爱用品 坂道どっとこむ",
    "desc": "nekokosyan.com",
    "detail": "・更新频率很低 不知道是否还在更新\n\n成员爱用品\n・耳机\n・相机\n・鞋子\n・化妆水\n・书籍・杂志\n・香水",
    "tags": [
      "item"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://nekokosyan.com/"
  },
  {
    "name": "⭐Sakamichi Blog Archive",
    "desc": "sakamichi.co",
    "detail": "・坂道成员博客图片保存网站\n・除乃木坂46外，存有成员个人历来的高画质公式照。\n　・乃木坂46：2017年7月 -\n　・欅坂46：开设（2015年11月）- 改名（2020年10月）\n　・日向坂46：けやき坂46として开设（2016年8月）-\n　・櫻坂46：改名（2020年10月）-\n\n・[坂道ブログ画像BOT / X](https://x.com/foxtrotal)\n・[公式博客月发布量统计/跳转](https://contributions.sakamichi.co/)",
    "tags": [
      "blog",
      "photo",
      "公式"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://www.sakamichi.co/"
  },
  {
    "name": "⭐乃木地图 | Nogimap",
    "desc": "nogimap.com",
    "detail": "Nogimap 是由社群共同维护的地图专案，希望让圣巡变得更方便。",
    "tags": [
      "place",
      "oshi"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://nogimap.com/"
  },
  {
    "name": "坂道系圣地巡礼地图",
    "desc": "[nekokosyan] google.com/maps",
    "detail": "・MV与写真集的外景地",
    "tags": [
      "place",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://www.google.com/maps/d/u/0/viewer?ll=38.00318233668623%2C139.5474794800408&z=6&mid=1uTzHlTnEyFZOgZVwoHkpe0ZhGzNlfT0"
  },
  {
    "name": "乃木坂46のロケ地まとめ",
    "desc": "tsuredure-nogi-blog.com",
    "detail": "・主要是MV拍摄外景地\n・最后更新时间 2025.07.27",
    "tags": [
      "place",
      "oshi"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://tsuredure-nogi-blog.com/rokechi-matome/"
  },
  {
    "name": "乃木旅",
    "desc": "nogitabi.blog.jp",
    "detail": "・坂道圣巡情报\n・生写真/个人公式照\n・[地图使用说明书（日文）](https://nogitabi.blog.jp/archives/admin_message.html)\n\n・[Instagram](https://www.instagram.com/nogitabi/)　・[X（Twitter）](https://x.com/nogitabi46)",
    "tags": [
      "place",
      "photo",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://nogitabi.blog.jp/"
  },
  {
    "name": "日向坂46　聖地一覧",
    "desc": "[清水ひなた] google.com/maps",
    "detail": "日向坂46的圣地一覧",
    "tags": [
      "place",
      "oshi"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://www.google.com/maps/d/u/0/viewer?mid=14Otcijw10dc-Fyu1P-q-BygLgiLK2sQ&ll=35.33666676792876%2C139.69073570374468&z=9"
  },
  {
    "name": "⭐Hinatamap 圣地巡礼地図",
    "desc": "[Nullヾ] hinatamap.pages.dev",
    "link": "https://hinatamap.pages.dev/",
    "detail": "专为日向坂46粉丝打造，优化 Google My Maps 体验：\n\n・实时动态同步：同步【清水ひなた】地图数据\n・多图资料导航：采用日本国土地理院图资\n・跳转 Google 地图导航\n・标签搜索",
    "tags": [
      "place",
      "oshi"
    ],
    "groups": [
      "hina"
    ]
  },
  {
    "name": "聖地巡礼したい",
    "desc": "tsuredure-nogi-blog.com",
    "detail": "外景、拍摄地介绍网站。\n・主要为MV拍摄的外景地\n\n・[圣地巡礼したい / X](https://x.com/junrei_shitai)",
    "tags": [
      "place",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://tsuredure-nogi-blog.com/"
  },
  {
    "name": "坂道ツールズ",
    "desc": "[圣地巡礼したい] tools.yotujoho.com",
    "detail": "・博客搜索 照片保存\n・官方新闻搜索\n・快速复制应募序号\n・生日一覧・比较",
    "tags": [
      "information",
      "tool",
      "blog"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://tools.yotujoho.com/"
  },
  {
    "name": "fumi Diary 2号店: ロケ地ノート",
    "desc": "blog.livedoor.jp/fumichen2",
    "detail": "櫻坂46 外景地",
    "tags": [
      "place",
      "oshi"
    ],
    "groups": [
      "saku"
    ],
    "link": "http://blog.livedoor.jp/fumichen2/tag/%E6%AB%BB%E5%9D%8246"
  },
  {
    "name": "乃木坂生写真のすゝめ-レート解析-",
    "desc": "nogizakarate.com",
    "detail": "・生写真资料\n・交换价格表\n・最后更新时间 2025.09.16",
    "tags": [
      "photo",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://nogizakarate.com/"
  },
  {
    "name": "高画質壁紙iPhone工事中",
    "desc": "iphone46.blog.jp",
    "detail": "・生写真\n・高画质壁纸",
    "tags": [
      "photo"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://iphone46.blog.jp/"
  },
  {
    "name": "日向坂辞典",
    "desc": "hinatadic.blog.jp",
    "detail": "・生写真",
    "tags": [
      "photo"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://hinatadic.blog.jp/"
  },
  {
    "name": "ぽむた/乃木坂ランキング",
    "desc": "x.com/pao_paobon",
    "detail": "各种杂项数据排行",
    "tags": [
      "sns",
      "blog",
      "streaming",
      "ranking"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://x.com/pao_paobon"
  },
  {
    "name": "UNI'S ON AIR 攻略Wiki",
    "desc": "gamerch.com/unis-on-air",
    "link": "https://gamerch.com/unis-on-air/",
    "detail": "",
    "tags": [
      "公式",
      "game",
      "oshi"
    ],
    "groups": [
      "saku",
      "hina"
    ]
  },
  {
    "name": "⭐ngzkHandshakeTable",
    "desc": "universallyleo.github.io/ngzkHandshakeTable",
    "detail": "仅有乃木坂46数据\n・完售表（个别・全国）\n・完售推移・比较\n・销售量数据\n・个人资料\n・邮件・Message统计功能",
    "tags": [
      "meet",
      "information",
      "sale",
      "message",
      "create",
      "tool"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://universallyleo.github.io/ngzkHandshakeTable/"
  },
  {
    "name": "⭐坂道完売表 Senu Blog",
    "desc": "senublog.com",
    "detail": "[せぬ / X](https://x.com/Senu_Blog)",
    "tags": [
      "meet"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://senublog.com/"
  },
  {
    "name": "⭐ミリアンペア(milli ampere)",
    "desc": "x.com/1milliampere",
    "detail": "坂道系MV播放量推移数据",
    "tags": [
      "song",
      "streaming",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/1milliampere"
  },
  {
    "name": "Unjustified",
    "desc": "x.com/unjustified1005",
    "detail": "坂道系各种数据汇总",
    "tags": [
      "song",
      "streaming",
      "live",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/unjustified1005"
  },
  {
    "name": "⭐たたん",
    "desc": "x.com/itsunogi46",
    "detail": "・及时完售表・应募・抽选结果数据汇总",
    "tags": [
      "meet"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/itsunogi46"
  },
  {
    "name": "⭐坂道の完売表",
    "desc": "x.com/nogi_kanbai",
    "detail": "・完売表・应募・抽选结果",
    "tags": [
      "meet"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/nogi_kanbai"
  },
  {
    "name": "坂道グループデータベース",
    "desc": "sakamichi-data.com",
    "detail": "・见面会完售表\n・成员个人资料\n・[KENT ◢|坂道データサイト運営 / X](https://x.com/kent_eeeee)",
    "tags": [
      "meet",
      "information",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://sakamichi-data.com/"
  },
  {
    "name": "乃木坂46データまとめ",
    "desc": "nogizaka46data.blogspot.com",
    "detail": "・电视・广播出演统计\n・歌曲资料",
    "tags": [
      "sns",
      "information",
      "song",
      "forum",
      "ranking"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://nogizaka46data.blogspot.com/"
  },
  {
    "name": "日向坂46･櫻坂46 Billboard",
    "desc": "x.com/h_s_46Billboard",
    "detail": "日向坂46･櫻坂46的告示牌相关情报，与Youtube、Oricon相关情报。",
    "tags": [
      "streaming",
      "ranking"
    ],
    "groups": [
      "saku",
      "hina"
    ],
    "link": "https://x.com/h_s_46Billboard"
  },
  {
    "name": "⭐乃木坂方丈記",
    "desc": "nekojitachamo.livedoor.blog",
    "detail": "乃木坂46 活动内容备忘录",
    "tags": [
      "information",
      "forum"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://nekojitachamo.livedoor.blog/"
  },
  {
    "name": "ヨヨりんご46",
    "desc": "x.com/yoyokaki46",
    "link": "https://x.com/yoyokaki46",
    "detail": "・乃木坂46 SNS转推\n・坂道杂志",
    "tags": [
      "公式",
      "sns",
      "blog",
      "photo"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ]
  },
  {
    "name": "沈黙した村人よ🍣",
    "desc": "x.com/makoto46_CYC",
    "detail": "・日向坂46专门\n・完售表・应募・抽选结果",
    "tags": [
      "meet"
    ],
    "groups": [
      "hina"
    ],
    "link": "https://x.com/makoto46_CYC"
  },
  {
    "name": "⭐Message统计分析",
    "desc": "msg.nogi46.me",
    "detail": "",
    "tags": [
      "message",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://msg.nogi46.me/stats"
  },
  {
    "name": "Message 爬取工具",
    "desc": "github.com",
    "link": "",
    "detail": "・[proshunsuke/colmsg](https://github.com/proshunsuke/colmsg)\n　・[ann-nogk/NogkSaver](https://github.com/ann-nogk/NogkSaver)：colmsgGUI\n・[limangox/sakamsg](https://github.com/limangox/sakamsg)\n・[easonyen16/b4nd-crawler](https://github.com/easonyen16/b4nd-crawler)",
    "tags": [
      "tool",
      "message"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ]
  },
  {
    "name": "こけし",
    "desc": "x.com/kks_slope",
    "detail": "乃木坂Message集计",
    "tags": [
      "message",
      "ranking"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://x.com/kks_slope"
  },
  {
    "name": "懐かしい",
    "desc": "x.com/__n____i",
    "link": "https://x.com/__n____i",
    "detail": "坂道杂学、不定期比对资料",
    "tags": [
      "song",
      "information",
      "ranking",
      "sale"
    ],
    "groups": [
      "nogi"
    ]
  },
  {
    "name": "けんしゅん",
    "desc": "x.com/kenshun46",
    "link": "https://x.com/kenshun46",
    "detail": "各种杂项统计、排行",
    "tags": [
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ]
  },
  {
    "name": "櫻坂46トーク集記",
    "desc": "x.com/sakuratotalling",
    "detail": "・[櫻坂46トーク集计 - YouTube](https://www.youtube.com/@sakuratotalling)",
    "tags": [
      "message",
      "ranking"
    ],
    "groups": [
      "saku"
    ],
    "link": "https://x.com/sakuratotalling"
  },
  {
    "name": "Sakurazaka Daily Data",
    "desc": "x.com/yamanteJr",
    "detail": "统计櫻坂46 推文 与前日比数据，毎天早上投稿。",
    "tags": [
      "sns",
      "ranking"
    ],
    "groups": [
      "saku"
    ],
    "link": "https://x.com/yamanteJr"
  },
  {
    "name": "データで見る櫻坂46",
    "desc": "x.com/sakurazakastats",
    "detail": "*更新频率很低\n\n📊 櫻坂46串流相关数据。 ",
    "tags": [
      "sns",
      "ranking"
    ],
    "groups": [
      "saku"
    ],
    "link": "https://x.com/sakurazakastats"
  },
  {
    "name": "⭐田代光輝",
    "desc": "x.com/mitsu0227",
    "detail": "写真集销售数据・预测・推移",
    "tags": [
      "sale",
      "photo",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/mitsu0227"
  },
  {
    "name": "坂道博客翻译",
    "desc": "46log.com",
    "detail": "・AI自动翻译博客网站\n・[博客统计分析](https://blog.sakamichi-tools.cn/stats)（暫時下線）\n・博客照片批量下载\n\n*通常情况下会比官方网站晚30分更新。 ",
    "tags": [
      "blog",
      "photo",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://46log.com/blog/"
  },
  {
    "name": "Instagram Archive",
    "desc": "46log.com",
    "detail": "成员Instagram内容存档。包含投稿的story・图片影片，并有批量下载功能，暂不包含投稿内文。 ",
    "tags": [
      "sns",
      "公式"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://46log.com/instagram/"
  },
  {
    "name": "🪦アレチボルト",
    "desc": "x.com/sabakunonezumi",
    "detail": "・MV的YouTube累积播放量推移\n・最后更新 2024.07.14",
    "tags": [
      "song",
      "streaming"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/sabakunonezumi"
  },
  {
    "name": "坂道もろもろデータ集",
    "desc": "x.com/WAJty8A2vuHn0Nn",
    "detail": "・各官方X《周间》追踪人数推移\n・最后更新 2025.11.02",
    "tags": [
      "sns",
      "information"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/WAJty8A2vuHn0Nn"
  },
  {
    "name": "乃木写（Nogisha）",
    "desc": "[けんしゅん] nogisha.app",
    "link": "https://nogisha.app/",
    "detail": "乃木坂生写真交换管理工具",
    "tags": [
      "oshi",
      "photo"
    ],
    "groups": [
      "nogi"
    ]
  },
  {
    "name": "UOA SSR+统整表",
    "desc": "[沉黙した村人よ] docs.google.com",
    "detail": "",
    "tags": [
      "game"
    ],
    "groups": [
      "saku",
      "hina"
    ],
    "link": "https://docs.google.com/spreadsheets/d/1RR2ixjmSL5sDPG7MA-Dk44GZrbaP7zbnMbP4qkjB7xs/edit?gid=0#gid=0"
  },
  {
    "name": "🪦握手会数据与情报统整",
    "desc": "ameblo.jp/seto-kasumi",
    "detail": "・过去的握手会完售资料，与博客留言数量等人气指标数据汇总。 \n・最后更新 2019.12.26",
    "tags": [
      "meet"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://ameblo.jp/seto-kasumi/"
  },
  {
    "name": "🪦NOGILIB",
    "desc": "nogilib.vercel.app",
    "detail": "截止到乃木坂46 31单的歌曲、站位、成员个人资料。",
    "tags": [
      "information",
      "song"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://nogilib.vercel.app/"
  },
  {
    "name": "🪦欅坂46表演总整理",
    "desc": "docs.google.com",
    "detail": "~2020-01-26",
    "tags": [
      "song",
      "live",
      "ranking"
    ],
    "groups": [
      "saku",
      "hina"
    ],
    "link": "https://docs.google.com/spreadsheets/d/1Qgkqi3t5ZX9cyOyBV7wXfnwNYR3fk40E1zj0O8il7RY/edit?gid=0#gid=0"
  },
  {
    "name": "⭐SBA Archive",
    "desc": "archive.sakamichi.co",
    "detail": "坂道系毕业生博客存档。",
    "tags": [
      "blog",
      "photo",
      "公式"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://archive.sakamichi.co/members"
  },
  {
    "name": "🪦毕业生博客存档（个人）",
    "desc": "欢迎提供其他网站",
    "detail": "・[桥本奈々未～山下美月](https://janelin612.github.io/n46-crawler/)\n・[岩瀬佑美子～寺田兰世](https://re-fort.net/NgzkBlogArchive/#/)\n・[今泉佑唯～渡辺梨加](https://re-fort.net/KykzkBlogArchive/#/)\n・[柿崎芽実・井口眞绪](https://re-fort.net/HntzkBlogArchive/#/)\n・个人\n　・[桥本奈々未](https://nondanee.github.io/onemoretime/nanami.hashimoto/)\n　・[久保史绪里](https://washioshihokubo.github.io/kubo-archive/)\n　・[梅澤美波](https://washioshihokubo.github.io/umezawa-archive/)",
    "tags": [
      "blog",
      "photo",
      "公式"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": ""
  },
  {
    "name": "⭐46◢ Online Meet Waiting Room",
    "desc": "status.meet.oshi-katsu.app",
    "detail": "线上见面会实时等待人数",
    "tags": [
      "公式",
      "meet",
      "ranking"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://status.meet.oshi-katsu.app/"
  },
  {
    "name": "⭐定点カメラお知らせbot",
    "desc": "x.com",
    "detail": "线上见面会的定点相机，成员出场自动检测通知bot📢\n\n・[乃木坂46](https://x.com/n46_teiten)\n・[櫻坂46](https://x.com/s46_teiten)\n・[日向坂46](https://x.com/h46_teiten)",
    "tags": [
      "streaming",
      "tool"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "##"
  },
  {
    "name": "⭐My選抜シミュレータ",
    "desc": "n46sim.com",
    "detail": "坂道系成员站位生成网站。\n\n・[乃木坂46](https://n46sim.com/)\n・[櫻坂46](https://sakura.n46sim.com/)\n・[日向坂46](https://hinata.n46sim.com/)",
    "tags": [
      "create",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://n46sim.com/"
  },
  {
    "name": "⭐坂道ソート",
    "desc": "sakamichi-sort.com",
    "detail": "乃木坂46、櫻坂46、日向坂46成员个人排行测验。",
    "tags": [
      "create",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://sakamichi-sort.com/"
  },
  {
    "name": "⭐N46Sorter",
    "desc": "n46sorter.com",
    "detail": "支援乃木坂46现役及毕业成员。透过二选一建立你的推し排名，还能重现选拔阵型。结果可保存为高画质分享图片。专为乃木坂粉丝打造的推しメン排序工具。",
    "tags": [
      "create",
      "oshi"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://n46sorter.com/ja"
  },
  {
    "name": "⭐握手Repo",
    "desc": "46log.com",
    "link": "https://46log.com/repo/create/",
    "detail": "生成并共享握手、见面会REPO",
    "tags": [
      "oshi",
      "create",
      "meet"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ]
  },
  {
    "name": "Repo手帐",
    "desc": "repo.zakazaka.top",
    "detail": "生成见面会REPO图",
    "tags": [
      "create",
      "oshi",
      "meet"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://www.repo.zakazaka.top/"
  },
  {
    "name": "MSG生成器",
    "desc": "46log.com",
    "detail": "成员MSG样式产生器，一键产生官方讯息格式图片",
    "tags": [
      "message",
      "create"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://46log.com/tools/msg-generator/"
  },
  {
    "name": "生写生成器",
    "desc": "46log.com",
    "detail": "可制作生写样式的图片。",
    "tags": [
      "photo",
      "create",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://46log.com/photocard/create/"
  },
  {
    "name": "⭐App「坂道ハウス」",
    "desc": "hinata-house-prod.web.app",
    "detail": "仅有iOS版本，安卓版本已不再维护。可实时推送官方新闻与博客消息，查看まとめ与成员个人资讯，SHOWROOM开播推送、SNS消息与网路新闻统合等功能。 \n\n【iOS】\n・[乃木ハウス](https://apps.apple.com/jp/app/%E4%B9%83%E6%9C%A8%E3%83%8F%E3%82%A6%E3%82%B9-for-%E4%B9%83%E6%9C%A8%E5%9D%8246/id776867932)\n・[樱ハウス](https://apps.apple.com/jp/app/%E6%AB%BB%E3%83%8F%E3%82%A6%E3%82%B9-for-%E6%AB%BB%E5%9D%8246/id1097369736)\n・[ひなたハウス](https://apps.apple.com/jp/app/%E3%81%B2%E3%81%AA%E3%81%9F%E3%83%8F%E3%82%A6%E3%82%B9-for-%E6%97%A5%E5%90%91%E5%9D%8246/id1457577580)",
    "tags": [
      "blog",
      "information",
      "photo",
      "forum",
      "tool",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://hinata-house-prod.web.app/"
  },
  {
    "name": "⭐App「Otaac」",
    "desc": "otaac.me",
    "detail": "由社群维护的偶像日程管理App。 ",
    "tags": [
      "tool",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://www.otaac.me/service/app"
  },
  {
    "name": "⭐坂道46まとめ",
    "desc": "博客相关工具",
    "detail": "App中包含最新新闻与博客更新及まとめ统整。 \n\n・[iOS](https://apps.apple.com/jp/app/%E5%9D%82%E9%81%9346%E3%81%BE%E3%81%A8%E3%82%81/id1508350322)・[Android](https://play.google.com/store/apps/details?id=stephennn.blogdownload)\n・[Telegram Bot](https://t.me/NogiBlog_bot)：另有Showroom开播通知(附m3u8鏈接)\n・Telegram 个别频道\n　・[乃木坂46ブログ](https://t.me/s/nogizaka_blog) 2022-03-02~\n　・[櫻坂46ブログ](https://t.me/s/keyakizaka46blog) 2020-07-08~\n　・[日向坂46ブログ](https://t.me/s/hinatazakablog) 2021-02-23~",
    "tags": [
      "公式",
      "sns",
      "blog",
      "streaming",
      "photo",
      "tool",
      "oshi"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://x.com/stephen64860726/status/1679410636902785024"
  },
  {
    "name": "博客通知／内容存档",
    "desc": "Telegram 频道",
    "detail": "・[乃木坂46 ブログ](https://t.me/s/Nogizaka46_blog) 2024-12-27~\n・[櫻坂46 ブログ](https://t.me/s/Sakurazaka46_Blog) 2025-10-10~\n・[日向坂46 ブログ](https://t.me/s/Hinatazaka46_Blog) 2025-10-09~",
    "tags": [
      "公式",
      "blog",
      "photo"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": ""
  },
  {
    "name": "Rakuten Ticket 速填工具",
    "desc": "rtkt.shouko.tw",
    "link": "https://rtkt.shouko.tw/",
    "detail": "",
    "tags": [
      "tool",
      "oshi"
    ],
    "groups": []
  },
  {
    "name": "showroom-player",
    "desc": "github.com",
    "link": "https://github.com/vacabun/showroom-player",
    "detail": "桌面端SHOWROOM串流播放器。内置留言、录制功能。",
    "tags": [
      "公式",
      "streaming",
      "tool"
    ],
    "groups": []
  },
  {
    "name": "SRDL",
    "desc": "github.com",
    "link": "https://github.com/Shijinzakaue/SRDL",
    "detail": "SHOWROOM 下载脚本",
    "tags": [
      "streaming",
      "tool"
    ],
    "groups": []
  },
  {
    "name": "⭐坂道工具箱",
    "desc": "[扩充功能] greasyfork.org",
    "detail": "・快速跳转博客翻译网站\n・官网资源获取\n・提供樱坂与日向坂官网快速回到顶部的功能\n・解除官网的限制\n・网页懒加载",
    "tags": [
      "tool"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://greasyfork.org/zh-TW/scripts/565367-%E5%9D%82%E9%81%93%E5%B7%A5%E5%85%B7%E7%AE%B1"
  },
  {
    "name": "SHOWROOM player",
    "desc": "[扩充功能] greasyfork.org",
    "link": "https://greasyfork.org/zh-TW/scripts/570529-showroom-player",
    "detail": "・SHOWROOM 直播间内浮窗\n・Potplayer快捷跳转\n・快速复制m3u8连结",
    "tags": [
      "streaming",
      "tool"
    ],
    "groups": []
  },
  {
    "name": "⭐米酱图片一键下载",
    "desc": "[扩充功能] chromewebstore.google.com",
    "detail": "一键批量下载当前新闻网页的相关图片（最大解析度）。 ",
    "tags": [
      "tool"
    ],
    "groups": [],
    "link": "https://chromewebstore.google.com/detail/modelpress-image-download/nepeemhbchedjbebdekhpkhknljppmif"
  },
  {
    "name": "Rajiko",
    "desc": "[扩充功能] chromewebstore.google.com",
    "detail": "Rajiko 是一款可以解除 radiko.jp 地理位置限制的工具！享受来自大阪、长崎，甚至东京、青森等海外地区的广播节目！ \n解锁 radiko.jp 的地理位置限制，收听来自日本各地以及日本国内和海外的广播电台。 ",
    "tags": [
      "tool"
    ],
    "groups": [],
    "link": "https://chromewebstore.google.com/detail/rajiko/ejcfdikabeebbgbopoagpabbdokepnff"
  },
  {
    "name": "TVAL now",
    "desc": "tval-now.switch-m.com",
    "detail": "日本电视台实时收视率",
    "tags": [
      "tool",
      "ranking",
      "公式"
    ],
    "groups": [],
    "link": "https://tval-now.switch-m.com/"
  },
  {
    "name": "番組表.Gガイド",
    "desc": "bangumi.org",
    "detail": "・热度排行\n・今日节目表\n　・[地上波](https://bangumi.org/epg/td)\n　・[BS](https://bangumi.org/epg/bs)\n　・[CS](https://bangumi.org/epg/cs)\n　・[ラジオ](https://bangumi.org/epg/radio)",
    "tags": [
      "ranking",
      "公式"
    ],
    "groups": [],
    "link": "https://bangumi.org/"
  },
  {
    "name": "电视出演时长排行",
    "desc": "tv-ranking.com",
    "detail": "本网站根据电视节目指南中列出的艺人信息，发布艺人出场时间和节目数量的排名。由于数据是根据电视节目指南计算得出，因此并不反映节目的受欢迎程度（收视率）。即使艺人出现在节目中，如果未在电视节目指南中列出，则不会被计入排名。此外，即使艺人出现，其在节目中的出场时长也无法确定。在本网站中，「电视节目指南中列出的艺人」指的是「对节目推广有价值」的艺人。我们希望您能将此视为衡量节目制作方对艺人价值的一种方式。 ",
    "tags": [
      "ranking",
      "公式"
    ],
    "groups": [],
    "link": "https://www.tv-ranking.com/"
  },
  {
    "name": "⭐日本广播播放器",
    "desc": "radio.nogi46.me",
    "detail": "・无限制实时广播收听\n・[存档回放功能](https://record.nogi46.me/table) record.nogi46.me",
    "tags": [
      "公式",
      "tool"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ],
    "link": "https://radio.nogi46.me/"
  },
  {
    "name": "坂道广播站",
    "desc": "46log.com",
    "link": "https://46log.com/radio/",
    "detail": "・实时广播\n・回放功能\n・樱耳串流\n・坂道广播排程",
    "tags": [
      "tool",
      "streaming",
      "公式"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ]
  },
  {
    "name": "有名人Instagram排行网",
    "desc": "insta.refetter.com",
    "detail": "可检索并显示Instagram帐号过往追踪人数推移",
    "tags": [
      "sns"
    ],
    "groups": [
      "公式"
    ],
    "link": "https://insta.refetter.com/"
  },
  {
    "name": "乃木坂46 卒業メンバー SNS",
    "desc": "Telegram 频道",
    "detail": "元·乃木坂46的SNS，与关联新闻的频道",
    "tags": [
      "公式",
      "sns",
      "information",
      "photo"
    ],
    "groups": [],
    "link": "https://t.me/nogizaka46_grad_SNS"
  },
  {
    "name": "COSTUME | 櫻坂46公式サイト",
    "desc": "sakurazaka46.com",
    "detail": "櫻坂46官方服裝",
    "tags": [
      "clothes",
      "公式"
    ],
    "groups": [
      "saku"
    ],
    "link": "https://sakurazaka46.com/s/s46/page/costume?ima=0000"
  },
  {
    "name": "東京パソコンクラブ",
    "desc": "unityroom.com",
    "detail": "节目内做的小游戏存档",
    "tags": [
      "公式",
      "game"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://unityroom.com/users/cnt54a3dio78skjxv9fe"
  },
  {
    "name": "FM-FUJI \"STUDIO ViViD\"",
    "desc": "twitcasting.tv",
    "detail": "FM FUJI 广播官方直播房间（无公开过往直播存档）",
    "tags": [
      "公式",
      "streaming"
    ],
    "groups": [
      "nogi"
    ],
    "link": "https://twitcasting.tv/c:fmfuji_twitcasting/"
  },
  {
    "name": "上行之坂字幕组",
    "desc": "ngzketm.top",
    "link": "https://ngzketm.top/",
    "detail": "",
    "tags": [
      "streaming"
    ],
    "groups": [
      "nogi"
    ]
  },
  {
    "name": "真的晒字幕组",
    "desc": "h46sub.cc",
    "link": "https://h46sub.cc/",
    "detail": "",
    "tags": [
      "streaming"
    ],
    "groups": [
      "hina"
    ]
  },
  {
    "name": "Aidoru Online",
    "desc": "aidoru-online.me",
    "link": "https://aidoru-online.me/index.php",
    "detail": "日本偶像种子资源网，请遵守互利原则使用。",
    "tags": [
      "公式",
      "streaming",
      "photo",
      "song"
    ],
    "groups": [
      "nogi",
      "saku",
      "hina"
    ]
  },
  {
    "name": "字幕汇出工具",
    "desc": "photocard.sakamichi-tools.cn",
    "detail": "・格式转换: 支援 JSON (CapCut/Whisper), SRT, ASS, TXT 多种格式的转换。 \n・字幕合并: 智慧合并时间轴相近的短句，优化断句，尤其针对日语语气词和短句进行了优化。 \n・文本处理: 提供大量删除日语语气词、文字替换等功能。 \n・[SRT/ASS 时间轴修复工具](https://photocard.sakamichi-tools.cn/srt_fixer.html)\n・[字幕FAD特效工具](https://photocard.sakamichi-tools.cn/fad_tool.html)",
    "tags": [
      "tool"
    ],
    "groups": [],
    "link": "https://photocard.sakamichi-tools.cn/subtitle.html"
  },
  {
    "name": "歌曲检定",
    "desc": "music-kentei.net",
    "detail": "歌曲熟悉度测验",
    "tags": [
      "oshi"
    ],
    "groups": [],
    "link": "https://music-kentei.net/"
  }
];

var tagLabels = {
  "公式": "公式",
  "item": "单品・爱用品",
  "clothes": "私服・衣服・制服",
  "place": "地点・圣地巡礼",
  "sns": "SNS",
  "blog": "博客",
  "meet": "握手・线握",
  "information": "个人・团体资料",
  "song": "歌曲资料",
  "sale": "销量",
  "game": "游戏",
  "streaming": "直播・串流",
  "photo": "照片・生写",
  "message": "Message",
  "create": "生成工具",
  "oshi": "推活相关",
  "ranking": "排行",
  "forum": "论坛・RSS・汇总",
  "live": "演唱会・歌单",
  "tool": "相关工具"
};