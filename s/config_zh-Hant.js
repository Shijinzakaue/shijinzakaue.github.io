var lastUpdated = "2026-03-14 23:25";

var officialConfig = [
  { name: "乃木坂46 官方網站", link: "https://www.nogizaka46.com", group: "nogi" },
  { name: "乃木坂46 新聞", link: "https://www.nogizaka46.com/s/n46/news/list", group: "nogi" },  
  { name: "乃木坂46 日程", link: "https://www.nogizaka46.com/s/n46/media/list", group: "nogi" },
  { name: "乃木坂46 成員", link: "https://www.nogizaka46.com/s/n46/search/artist", group: "nogi" },
  { name: "乃木坂46 部落格", link: "https://www.nogizaka46.com/s/n46/diary/MEMBER", group: "nogi" },
  { name: "OFFICIAL WEB SHOP", link: "https://www.nogizaka46shop.com/", group: "nogi" },
  { name: "乃木坂46 Mobile", link: "https://sp.nogizaka46.com", group: "nogi" }, 
  { name: "乃木動畫", link: "https://nogidoga.com/", group: "nogi" },
  { name: "MYNOGI", link: "https://mynogi.snft.jp/", group: "nogi" },
  { name: "線握/CD購入活動日程", link: "https://www.nogizaka46.com/s/n46/live?ima=0000&ct=HANDSHAKE", group: "nogi" },
  { name: "乃木坂46 Message", link: "https://message.nogizaka46.com/", group: "nogi" },
  { name: "乃木坂工事中", link: "https://tv-aichi.co.jp/nogi-kou/", group: "nogi" },
	
  { name: "櫻坂46 官方網站", link: "https://sakurazaka46.com/s/s46/", group: "saku" },  
  { name: "櫻坂46 新聞", link: "https://sakurazaka46.com/s/s46/news/list", group: "saku" },
  { name: "櫻坂46 日程", link: "https://sakurazaka46.com/s/s46/media/list", group: "saku" },
  { name: "櫻坂46 成員", link: "https://sakurazaka46.com/s/s46/search/artist", group: "saku" },
  { name: "櫻坂46 部落格", link: "https://sakurazaka46.com/s/s46/diary/blog/list", group: "saku" },
  { name: "OFFICIAL GOODS STORE", link: "https://store.plusmember.jp/sakurazaka46/", group: "saku" },
  { name: "MY PAGE", link: "https://sakurazaka46.com/s/s46/mypage", group: "saku" },
  { name: "經紀人日記", link: "https://sakurazaka46.com/s/s46/diary/managers_diary/list", group: "saku" },
  { name: "櫻耳廣播", link: "https://sakurazaka46.com/s/s46/diary/radio", group: "saku" },
  { name: "見面會日程", link: "https://sakurazaka46.com/s/s46/diary/event_page/list?ima=0000", group: "saku" },
  { name: "櫻坂46 Message", link: "https://message.sakurazaka46.com/", group: "saku" },
  { name: "轉角就是櫻坂？", link: "https://www.tv-tokyo.co.jp/keyaki/", group: "saku" },

  { name: "日向坂46 官方網站", link: "https://www.hinatazaka46.com/s/official/", group: "hina" },
  { name: "日向坂46 新聞", link: "https://www.hinatazaka46.com/s/official/news/list", group: "hina" },
  { name: "日向坂46 日程", link: "https://www.hinatazaka46.com/s/official/media/list", group: "hina" },
  { name: "日向坂46 成員", link: "https://www.hinatazaka46.com/s/official/search/artist", group: "hina" },
  { name: "日向坂46 部落格", link: "https://www.hinatazaka46.com/s/official/diary/member", group: "hina" },
  { name: "OFFICIAL GOODS STORE", link: "https://store.plusmember.jp/hinatazaka46/", group: "hina" },
  { name: "我的頁面", link: "https://www.hinatazaka46.com/s/official/mypage", group: "hina" },
  { name: "經紀人日記", link: "https://www.hinatazaka46.com/s/official/diary/manager/list", group: "hina" },
  { name: "日向坂46 fanclub", link: "https://www.hinatazaka46.com/s/official/page/about_fanclub", group: "hina" },
  { name: "見面會日程", link: "https://www.hinatazaka46.com/s/official/diary/event/list?ima=0000", group: "hina" },
  { name: "日向坂46 Message", link: "https://message.hinatazaka46.com/", group: "hina" },
  { name: "在日向坂見面吧", link: "https://www.tv-tokyo.co.jp/hinatazaka/", group: "hina" },

];

var toolConfig = [
{
  name: "⭐乃木坂46 SNS",
  desc: "Telegram 頻道",
  detail: "・[乃木坂46 OFFICIAL YouTube CHANNEL](https://www.youtube.com/@nogizaka46SMEJ)\n・[乃木坂配信中](https://www.youtube.com/@nogizakahaishinchu)\n・[X（旧Twitter）](https://x.com/nogizaka46)\n・[Instagram](https://www.instagram.com/nogizaka46_official/)\n・[Facebook](https://www.facebook.com/nogizaka46)\n・[TikTok](https://www.tiktok.com/@nogizaka46_official)\n・[Line](https://lin.ee/EwCV3ub)\n・[Weibo](https://weibo.com/u/6372196907)\n・[Bilibili](https://space.bilibili.com/603028270)",
  tags: ["公式","sns","blog","information","photo"],
  groups: ["nogi"],
  link: "https://t.me/s/nogizaka46_SNS/"
},
{
  name: "⭐櫻坂46 SNS",
  desc: "Telegram 頻道",
  detail: "・[櫻坂46 OFFICIAL YouTube CHANNEL](https://www.youtube.com/@sakurazaka46SMEJ)\n・[櫻坂チャンネル](https://www.youtube.com/@sakurazakachannel/)\n・[X（舊Twitter）](https://x.com/sakurazaka46)\n・[Instagram @sakurazaka46jp](https://www.instagram.com/sakurazaka46jp)\n・[Instagram @sakurazaka46_info_official](https://www.instagram.com/sakurazaka46_info_official/)\n・[TikTok](https://www.tiktok.com/@sakurazaka46.officialtk)",
  tags: ["公式","sns","blog","information","photo"],
  groups: ["saku"],
  link: "https://t.me/s/sakurazaka46_sns/"
},
{
  name: "⭐日向坂46 SNS",
  desc: "Telegram 頻道",
  detail: "・[日向坂46 OFFICIAL YouTube CHANNEL](https://www.youtube.com/@46officialyoutubechannel99)\n・[日向坂ちゃんねる](https://www.youtube.com/@hinatazakachannel)\n・[X（舊Twitter）](https://x.com/hinatazaka46)\n・[Instagram](https://www.instagram.com/hinatazaka46)\n・[TikTok](https://www.tiktok.com/@hinatazakanews)",
  tags: ["公式","sns","blog","information","photo"],
  groups: ["hina"],
  link: "https://t.me/s/hinatazaka46_sns/"
},
{
  name: "⭐坂道46LOVER",
  desc: "nogi46p.com",
  detail: "坂道最新情報站\n・登場・出演情報\n・網路新聞・官方新聞\n\n・[坂道46LOVER ◢⁴⁶ / X](https://x.com/nogi46press)\n・RSSLink：[https://nogi46p.com/feed/rdf](https://nogi46p.com/feed)",
  tags: ["公式","information","forum"],
  groups: ["nogi","saku","hina"],
  link: "https://nogi46p.com/"
},
{
  name: "⭐坂道ちゃんねる",
  desc: "sakamichi.f-frontier.com",
  detail: "坂道相關新聞情報站\n・官方部落格\n・每日日程彙總\n・網路新聞・官方新聞\n\n・[◢ 坂道推し / X](https://x.com/sakamichi__oshi)\n・RSSLink：[https://sakamichi.f-frontier.com/feed/rdf](https://sakamichi.f-frontier.com/feed)",
  tags: ["公式","information","forum"],
  groups: ["nogi","saku","hina"],
  link: "https://sakamichi.f-frontier.com/"
},
  {
    name: "⭐乃木坂46データベース",
    desc: "n46db.com",
    detail: "坂道成員個人資料與参加歌曲、歌曲站位、作曲家・編曲家、MV與個人PV導演等情報資訊彙總。\n\n・[About this website](https://n46db.com/about.php)",
    tags: ["information", "song", "photo", "create"],
    groups: ["nogi", "saku", "hina"],
    link: "https://n46db.com/"
  },
{
  name: "坂道雑文帳",
  desc: "meaning-of-goodbye.com",
  detail: "個人經營的坂道系部落格。\n歌曲站位與演唱會歌單等，彙總系文章。\n相關工具：[坂道シリーズ 統合データベース検索](https://meaning-of-goodbye.com/sakamichi-search/)\n\n・[關於此網站](https://meaning-of-goodbye.com/about/)",
  tags: ["song","forum","live"],
  groups: ["nogi","saku","hina"],
  link: "https://meaning-of-goodbye.com/"
},
{
  name: "⭐坂道 統合データベース検索",
  desc: "[坂道雑文帳] Sakamichi Universe",
  detail: "「坂道雑文帳」運營的坂道系情報檢索系統。\n成員、歌曲、演唱會、站位、各種資訊等，以及最新活動與發售情報皆可搜索。\n\n・[乃木坂46 演唱會表演過的歌曲總表](https://docs.google.com/spreadsheets/d/1Kxr5ScgjxmGPlP-UohWkQ47sCKxAM5rm-z_aAA4Uerk/edit?gid=0#gid=0)\n・[網站介紹文章](https://note.com/nogikeyaksh/n/n95f652f7d114)",
  tags: ["information","song","live"],
  groups: ["nogi","saku","hina"],
  link: "https://meaning-of-goodbye.com/sakamichi-search/"
},
{
  name: "坂道データベース",
  desc: "sakamichidatabase.penguinelegy.com",
  detail: "・最新日程\n・個人與團體彙總資料\n・歌曲資料",
  tags: ["sns","information","song","live","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://sakamichidatabase.penguinelegy.com/"
},
{
  name: "⭐あの坂道を登れ",
  desc: "anosaka.com",
  detail: "・演唱會與活動參加REPO\n・成員出演媒體感想\n・人氣排行\n・遊戲攻略\n\n・[あのさか / X](https://x.com/anosaka_46)：相關資料收集・分析\n・RSSLink：[https://anosaka.com/feed](https://anosaka.com/feed)\n・[ABOUT](https://anosaka.com/about-us)",
  tags: ["meet","information","song","sale","create","forum","live","game","ranking","sns","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://anosaka.com/"
},
{
  name: "坂道データベース",
  desc: "[あの坂道] sakamichidb.anosaka.com",
  detail: "「あの坂道」運營的坂道系MV播放量統計非官方網站。\n\n・[關於此網站](https://sakamichidb.anosaka.com/other/about)",
  tags: ["information","song","streaming","create","tool","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://sakamichidb.anosaka.com/"
},
{
  name: "⭐坂道シリーズ楽曲DB",
  desc: "saka46.fun",
  detail: "坂道系歌曲資料彙總網站（作曲家、MV導演、舞蹈老師等）。",
  tags: ["song","streaming","live","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://saka46.fun/"
},
{
  name: "乃木坂46まとめ",
  desc: "nogizaka46.u-ff.com",
  detail: "[乃木坂46データベース](https://nogizaka46.u-ff.com/database/)\n・最新新聞\n・成員與團體資料彙總\n・活動歌單\n・見面會資料\n・村外影片彙總",
  tags: ["clothes","meet","information","song","sale","forum","live","ranking"],
  groups: ["nogi"],
  link: "https://nogizaka46.u-ff.com/"
},
{
  name: "日向坂46まとめ",
  desc: "hinatazaka46.u-ff.com",
  detail: "[日向坂46データベース](https://hinatazaka46.u-ff.com/database/)\n・最新新聞\n・成員與團體資料彙總\n・活動歌單\n・見面會資料\n・影片連結（Smartnews）\n・Podcast（Audee）",
  tags: ["clothes","meet","information","song","sale","streaming","forum","live","ranking"],
  groups: ["hina"],
  link: "https://hinatazaka46.u-ff.com/"
},
{
  name: "まいにち日向坂",
  desc: "hinatafan.com",
  detail: "・演唱會歌單\n・日向坂46聖地資料\n・成員資料彙總",
  tags: ["place","meet","information","song","forum","live","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://hinatafan.com/"
},
{
  name: "坂道46データベース",
  desc: "zaka46db.com",
  detail: "・團體資料\n・個人情報\n・寫真集資料\n・歌曲資料\n・學力測驗資料",
  tags: ["sns","information","song"],
  groups: ["nogi","saku","hina"],
  link: "https://zaka46db.com/"
},
{
  name: "新時代レポ",
  desc: "report-newage.com",
  detail: "・[乃木坂46 歌單](https://report-newage.com/?s=%E4%B9%83%E6%9C%A8%E5%9D%8246)\n・[櫻坂46 歌單](https://report-newage.com/?s=%E6%AB%BB%E5%9D%8246)\n・[日向坂46 歌單](https://report-newage.com/?s=%E6%97%A5%E5%90%91%E5%9D%8246)",
  tags: ["live"],
  groups: ["nogi","saku","hina"],
  link: "https://report-newage.com/"
},
{
  name: "SAKURA DATA 46",
  desc: "sakurazaka46.live",
  detail: "成員個資與暱稱、生日、血型、身長等詳細情報。也把歌曲資料（單曲・專輯・原表演成員・Center次數・作詞作曲者）整理的更加直觀。\n最新單曲情報與畢業成員的資料也正在更新中。",
  tags: ["information","song"],
  groups: ["saku"],
  link: "https://sakurazaka46.live/"
},
{
  name: "HINABASE",
  desc: "kasumizaka46.com",
  detail: "成員個資與暱稱、生日、血型、身長等詳細情報。也把歌曲資料（單曲・專輯・原表演成員・Center次數・作詞作曲者）整理的更加直觀。\n最新單曲情報與畢業成員的資料也正在更新中。",
  tags: ["information","song"],
  groups: ["hina"],
  link: "https://kasumizaka46.com/"
},
{
  name: "坂道系直播時間序資料庫",
  desc: "wikihouse.com/sakamichi",
  detail: "坂道系的Showroom、SNS等直播影片線上觀看統整網站。",
  tags: ["information","streaming","sns"],
  groups: ["nogi","saku","hina"],
  link: "https://www.wikihouse.com/sakamichi/index.php?FrontPage"
},
{
  name: "ファン向けの応援ブログ「がなぷら」",
  desc: "hiragana-plan.com",
  detail: "【坂道/符號/PassCode/MFS】的個人經營部落格「がなぷら」。個人資料與排行等資料彙總。",
  tags: ["information","forum","ranking","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://hiragana-plan.com/"
},
{
  name: "偶像寵物名彙總",
  desc: "seesaawiki.jp/nameofidolsdog",
  detail: "資料不完全、不確定是否還在更新",
  tags: ["information"],
  groups: ["nogi","saku","hina"],
  link: "https://seesaawiki.jp/nameofidolsdog/"
},
{
  name: "日向坂46　演唱會情報",
  desc: "w.atwiki.jp/hinatazaka46liveinfo",
  detail: "粉絲建立的日向坂46演唱會相關問題與資訊的WIKI網站。",
  tags: ["place","sns","information","song","live","oshi"],
  groups: ["hina"],
  link: "https://w.atwiki.jp/hinatazaka46liveinfo/"
},
{
  name: "乃木坂パープルハート",
  desc: "nogizaka.info",
  detail: "乃木坂46新粉絲向資訊站，內涵乃木坂46過往的小知識與雜學。",
  tags: ["clothes","information","song","oshi"],
  groups: ["nogi"],
  link: "https://www.nogizaka.info/index.html"
},
{
  name: "ZERO スケジュールまとめ",
  desc: "x.com/zero00zero46",
  detail: "製作未來的日程總覽（圖片形式）的Ｘ大佬",
  tags: ["公式","information","oshi"],
  groups: ["nogi"],
  link: "https://x.com/zero00zero46"
},
{
  name: "⭐ 46 NEWS !",
  desc: "46news.net",
  detail: "日本論壇文章彙總網站。\n來源 talk.jp->各大まとめ網站->彙總網。\n\n以下為我自行製作的RSS，更新頻率為0~30分不等。\nRSSLink politepol.com\n・乃木坂46 [/fd/wDNr0GJnT4W6.xml](https://politepol.com/fd/wDNr0GJnT4W6.xml)\n・櫻坂46　 [/fd/tQUjLrmQlSYk.xml](https://politepol.com/fd/tQUjLrmQlSYk.xml)\n・日向坂46 [/fd/1ROHnysso4vs.xml](https://politepol.com/fd/1ROHnysso4vs.xml)",
  tags: ["forum"],
  groups: ["nogi","saku","hina"],
  link: "https://46news.net/"
},
{
  name: "⭐SHOWROOMデータ",
  desc: "aknog.github.io/srdata",
  detail: "坂道系SHOWROOM直播數據。\n\n・[SHOWROOMデータ / X](https://x.com/srdxsa)\n・Smartnews影片連結（放課後談話等）",
  tags: ["streaming","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://aknog.github.io/srdata/"
},
{
  name: "雑誌封面情報（偶像）",
  desc: "x.com/otdmqivibpkq3so",
  detail: "",
  tags: ["公式","photo"],
  groups: [],
  link: "https://x.com/otdmqivibpkq3so"
},
{
  name: "⭐坂道 store",
  desc: "sakamichi-store.net",
  detail: "介紹坂道成員私服・鞋子・相機・化妝品的網站。來源通常是blog/Youtube /TV/TikTok的情報🌷少量聖地巡禮情報。 ※只由個人特定所以有可能有錯誤還請見諒。\n\n・[坂道store / X](https://x.com/sakamichi_store)",
  tags: ["item","clothes","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://sakamichi-store.net/"
},
{
  name: "⭐N46 fashion│私服･衣装",
  desc: "instagram.com/ngz46.fashionsnap",
  detail: "・私服\n・衣装\n・私物\n・單品\n・愛用品",
  tags: ["item","clothes","oshi"],
  groups: ["nogi"],
  link: "https://www.instagram.com/ngz46.fashionsnap/"
},
{
  name: "⭐pyon アイドル私服衣装特定",
  desc: "x.com/hntzk_46pyo",
  detail: "偶像私服 / 衣装 / 化妝品 / 店家 etc 特定彙總推文🐻❣️",
  tags: ["item","clothes"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/hntzk_46pyo"
},
{
  name: "⭐ひまり│坂道コスメ",
  desc: "x.com/himari_cosme_46",
  detail: "坂道系(主乃木坂)的愛用化妝品、護膚品、護髮品彙總 ˗ˋˏ ♡ ˎˊ˗ 有時有48、符號系、早安、iLiFE!的資訊 也會發之前的資訊 如有錯誤請見諒",
  tags: ["item","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/himari_cosme_46"
},
{
  name: "アイドルバケット",
  desc: "idolbucket.jp",
  detail: "・私服・衣装特定\n・演唱會歌單\n・應援手燈顏色",
  tags: ["item","clothes","information","song","live","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://idolbucket.jp/"
},
{
  name: "ゆにめいと Yunimate",
  desc: "yunimate.jp",
  detail: "坂道（乃木坂46・櫻坂46・欅坂46・日向坂46）衣服情報",
  tags: ["clothes","song"],
  groups: ["nogi","saku","hina"],
  link: "https://yunimate.jp/"
},
{
  name: "乃木坂46 N46ネットワーク",
  desc: "nogizaka46.net",
  detail: "・乃木坂46 演唱會歌單\n・歌曲情報\n・[RSS link](https://nogizaka46.net/feed/atom)",
  tags: ["clothes","information","song","forum","live"],
  groups: ["nogi"],
  link: "https://nogizaka46.net/"
},
{
  name: "JOYFUL LOVE 虹色作戦まとめwiki",
  desc: "w.atwiki.jp/joyfullove",
  detail: "日向坂46的JOYFUL LOVE 虹色作戰資訊彙總wiki。",
  tags: ["live","oshi"],
  groups: ["hina"],
  link: "https://w.atwiki.jp/joyfullove/"
},
{
  name: "乃木坂コール中",
  desc: "nogizaka46-live.hatenablog.jp",
  detail: "乃木坂46 歌曲打call資料站",
  tags: ["song","live","oshi"],
  groups: ["nogi"],
  link: "https://nogizaka46-live.hatenablog.jp/"
},
{
  name: "乃木坂紹介中",
  desc: "n46hako.com",
  detail: "基本上只有乃木坂46 的演唱會歌單",
  tags: ["song","live"],
  groups: ["nogi"],
  link: "https://n46hako.com/"
},
{
  name: "SEKAIWEEK 坂道シリーズ",
  desc: "the0ries.com",
  detail: "坂道系資訊彙總文章",
  tags: ["information","song","sale","forum"],
  groups: ["nogi","saku","hina"],
  link: "https://the0ries.com/category/nogizaka/"
},
{
  name: "坂道愛用品 坂道どっとこむ",
  desc: "nekokosyan.com",
  detail: "・更新頻率很低 不知道是否還在更新\n\n成員愛用品\n・耳機\n・相機\n・鞋子\n・化妝水\n・書籍・雜誌\n・香水",
  tags: ["item"],
  groups: ["nogi","saku","hina"],
  link: "https://nekokosyan.com/"
},
{
  name: "⭐乃木地圖 | Nogimap",
  desc: "nogimap.com",
  detail: "Nogimap 是由社群共同維護的地圖專案，希望讓聖巡變得更方便。",
  tags: ["place","oshi"],
  groups: ["nogi"],
  link: "https://nogimap.com/"
},
{
  name: "坂道系聖地巡禮地圖",
  desc: "[nekokosyan] google.com/maps",
  detail: "・MV與寫真集的外景地",
  tags: ["place","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://www.google.com/maps/d/u/0/viewer?ll=38.00318233668623%2C139.5474794800408&z=6&mid=1uTzHlTnEyFZOgZVwoHkpe0ZhGzNlfT0"
},
{
  name: "乃木坂46のロケ地まとめ",
  desc: "tsuredure-nogi-blog.com",
  detail: "・主要是MV拍攝外景地\n・最後更新時間 2025.07.27",
  tags: ["place","oshi"],
  groups: ["nogi"],
  link: "https://tsuredure-nogi-blog.com/rokechi-matome/"
},
{
  name: "乃木旅",
  desc: "nogitabi.blog.jp",
  detail: "・坂道聖巡情報\n・生寫真/個人公式照\n・[地圖使用說明書（日文）](https://nogitabi.blog.jp/archives/admin_message.html)\n\n・[Instagram](https://www.instagram.com/nogitabi/)　・[X（Twitter）](https://x.com/nogitabi46)",
  tags: ["place","photo","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://nogitabi.blog.jp/"
},
{
  name: "⭐日向坂46　聖地一覧",
  desc: "[清水ひなた] google.com/maps",
  detail: "日向坂46的聖地一覧",
  tags: ["place","oshi"],
  groups: ["hina"],
  link: "https://www.google.com/maps/d/u/0/viewer?mid=14Otcijw10dc-Fyu1P-q-BygLgiLK2sQ&ll=35.33666676792876%2C139.69073570374468&z=9"
},
{
  name: "聖地巡礼したい",
  desc: "tsuredure-nogi-blog.com",
  detail: "外景、拍攝地介紹網站。\n・主要為MV拍攝的外景地\n\n・[聖地巡礼したい / X](https://x.com/junrei_shitai)",
  tags: ["place","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://tsuredure-nogi-blog.com/"
},
{
  name: "坂道ツールズ",
  desc: "[聖地巡礼したい] tools.yotujoho.com",
  detail: "・部落格搜索 照片保存\n・官方新聞搜索\n・快速複製應募序號\n・生日一覧・比較",
  tags: ["information","tool","blog"],
  groups: ["nogi","saku","hina"],
  link: "https://tools.yotujoho.com/"
},
{
  name: "⭐Sakamichi Blog Archive",
  desc: "sakamichi.co",
  detail: "・坂道成員部落格圖片保存網站\n・除乃木坂46外，存有成員個人歷來的高畫質公式照。\n　・乃木坂46：2017年7月 -\n　・欅坂46：開設（2015年11月）- 改名（2020年10月）\n　・日向坂46：けやき坂46として開設（2016年8月）-\n　・櫻坂46：改名（2020年10月）-\n\n・[坂道ブログ画像BOT / X](https://x.com/foxtrotal)",
  tags: ["blog","photo","公式"],
  groups: ["nogi","saku","hina"],
  link: "https://www.sakamichi.co/"
},
{
  name: "fumi Diary 2号店: ロケ地ノート",
  desc: "blog.livedoor.jp/fumichen2",
  detail: "櫻坂46 外景地",
  tags: ["place","oshi"],
  groups: ["saku"],
  link: "http://blog.livedoor.jp/fumichen2/tag/%E6%AB%BB%E5%9D%8246"
},
{
  name: "乃木坂生写真のすゝめ-レート解析-",
  desc: "nogizakarate.com",
  detail: "・生寫真資料\n・交換價格表\n・最後更新時間 2025.09.16",
  tags: ["photo","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://nogizakarate.com/"
},
{
  name: "高画質壁紙iPhone工事中",
  desc: "iphone46.blog.jp",
  detail: "・生写真\n・高画質壁紙",
  tags: ["photo"],
  groups: ["nogi"],
  link: "https://iphone46.blog.jp/"
},
{
  name: "日向坂辞典",
  desc: "hinatadic.blog.jp",
  detail: "・生写真",
  tags: ["photo"],
  groups: ["hina"],
  link: "https://hinatadic.blog.jp/"
},
{
  name: "ぽむた/乃木坂ランキング",
  desc: "x.com/pao_paobon",
  detail: "各種雜項數據排行",
  tags: ["sns","blog","streaming","ranking"],
  groups: ["nogi"],
  link: "https://x.com/pao_paobon"
},
{
  name: "⭐ngzkHandshakeTable",
  desc: "universallyleo.github.io/ngzkHandshakeTable",
  detail: "僅有乃木坂46數據\n・完售表（個別・全国）\n・完售推移・比較\n・銷售量數據\n・個人資料\n・郵件・Message統計功能",
  tags: ["meet","information","sale","message","create","tool"],
  groups: ["nogi"],
  link: "https://universallyleo.github.io/ngzkHandshakeTable/"
},
{
  name: "⭐坂道完売表 Senu Blog",
  desc: "senublog.com",
  detail: "[せぬ / X](https://x.com/Senu_Blog)",
  tags: ["meet"],
  groups: ["nogi","saku","hina"],
  link: "https://senublog.com/"
},
{
  name: "⭐ミリアンペア(milli ampere)",
  desc: "x.com/1milliampere",
  detail: "坂道系MV播放量推移數據",
  tags: ["song","streaming","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/1milliampere"
},
{
  name: "Unjustified",
  desc: "x.com/unjustified1005",
  detail: "坂道系各種數據彙總",
  tags: ["song","streaming","live","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/unjustified1005"
},
{
  name: "⭐たたん",
  desc: "x.com/itsunogi46",
  detail: "・及時完售表・應募・抽選結果數據彙總",
  tags: ["meet"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/itsunogi46"
},
{
  name: "⭐坂道の完売表",
  desc: "x.com/nogi_kanbai",
  detail: "・完売表・應募・抽選結果",
  tags: ["meet"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/nogi_kanbai"
},
{
  name: "坂道グループデータベース",
  desc: "sakamichi-data.com",
  detail: "・見面會完售表\n・成員個人資料",
  tags: ["meet","information","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://sakamichi-data.com/"
},
{
  name: "乃木坂46データまとめ",
  desc: "nogizaka46data.blogspot.com",
  detail: "・電視・廣播出演統計\n・歌曲資料",
  tags: ["sns","information","song","forum","ranking"],
  groups: ["nogi"],
  link: "https://nogizaka46data.blogspot.com/"
},
{
  name: "日向坂46･櫻坂46 Billboard",
  desc: "x.com/h_s_46Billboard",
  detail: "日向坂46･櫻坂46的告示牌相關情報，與Youtube、Oricon相關情報。",
  tags: ["streaming","ranking"],
  groups: ["saku","hina"],
  link: "https://x.com/h_s_46Billboard"
},
{
  name: "⭐乃木坂方丈記",
  desc: "nekojitachamo.livedoor.blog",
  detail: "乃木坂46 活動內容備忘録",
  tags: ["information","forum"],
  groups: ["nogi"],
  link: "https://nekojitachamo.livedoor.blog/"
},
{
  name: "沈黙した村人よ🍣",
  desc: "x.com/makoto46_CYC",
  detail: "・日向坂46專門\n・完售表・應募・抽選結果",
  tags: ["meet"],
  groups: ["hina"],
  link: "https://x.com/makoto46_CYC"
},
{
  name: "⭐Message統計分析",
  desc: "msg.nogi46.me",
  detail: "",
  tags: ["message","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://msg.nogi46.me/stats"
},
{
  name: "こけし",
  desc: "x.com/kks_slope",
  detail: "乃木坂Message集計",
  tags: ["message","ranking"],
  groups: ["nogi"],
  link: "https://x.com/kks_slope"
},
{
  name: "櫻坂46トーク集計",
  desc: "x.com/sakuratotalling",
  detail: "・[櫻坂46トーク集計 - YouTube](https://www.youtube.com/@sakuratotalling)",
  tags: ["message","ranking"],
  groups: ["saku"],
  link: "https://x.com/sakuratotalling"
},
{
  name: "Sakurazaka Daily Data",
  desc: "x.com/yamanteJr",
  detail: "統計櫻坂46 推文 與前日比數據，毎天早上投稿。",
  tags: ["sns","ranking"],
  groups: ["saku"],
  link: "https://x.com/yamanteJr"
},
{
  name: "データで見る櫻坂46",
  desc: "x.com/sakurazakastats",
  detail: "*更新頻率很低\n\n📊 櫻坂46串流相關數據。",
  tags: ["sns","ranking"],
  groups: ["saku"],
  link: "https://x.com/sakurazakastats"
},
{
  name: "⭐田代光輝",
  desc: "x.com/mitsu0227",
  detail: "寫真集銷售數據・預測・推移",
  tags: ["sale","photo","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/mitsu0227"
},
{
  name: "坂道部落格翻譯",
  desc: "blog.sakamichi-tools.cn",
  detail: "・AI自動翻譯部落格網站\n・[部落格統計分析](https://blog.sakamichi-tools.cn/stats)\n・部落格照片批量下載\n\n*通常情況下會比官方網站晚30分更新。",
  tags: ["blog","photo","ranking"],
  groups: ["nogi","saku","hina"],
  link: "https://blog.sakamichi-tools.cn/#all"
},
{
  name: "Instagram Archive",
  desc: "ins.sakamichi-tools.cn",
  detail: "成員Instagram內容存檔。包含投稿的story・圖片影片，並有批量下載功能，暫不包含投稿內文。",
  tags: ["sns","公式"],
  groups: ["nogi","saku","hina"],
  link: "https://ins.sakamichi-tools.cn/"
},
{
  name: "🪦アレチボルト",
  desc: "x.com/sabakunonezumi",
  detail: "・MV的YouTube累積播放量推移\n・最後更新 2024.07.14",
  tags: ["song","streaming"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/sabakunonezumi"
},
{
  name: "坂道もろもろデータ集",
  desc: "x.com/WAJty8A2vuHn0Nn",
  detail: "・各官方X《週間》追蹤人數推移\n・最後更新 2025.11.02",
  tags: ["sns","information"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/WAJty8A2vuHn0Nn"
},
{
  name: "UOA SSR+統整表",
  desc: "[沈黙した村人よ] docs.google.com",
  detail: "",
  tags: ["game"],
  groups: ["saku","hina"],
  link: "https://docs.google.com/spreadsheets/d/1RR2ixjmSL5sDPG7MA-Dk44GZrbaP7zbnMbP4qkjB7xs/edit?gid=0#gid=0"
},
{
  name: "🪦握手會數據與情報統整",
  desc: "ameblo.jp/seto-kasumi",
  detail: "・過去的握手会完售資料，與部落格留言數量等人氣指標數據彙總。\n・最後更新 2019.12.26",
  tags: ["meet"],
  groups: ["nogi","saku","hina"],
  link: "https://ameblo.jp/seto-kasumi/"
},
{
  name: "🪦NOGILIB",
  desc: "nogilib.vercel.app",
  detail: "截止到乃木坂46 31單的歌曲、站位、成員個人資料。",
  tags: ["information","song"],
  groups: ["nogi"],
  link: "https://nogilib.vercel.app/"
},
{
  name: "🪦欅坂46表演總整理",
  desc: "docs.google.com",
  detail: "~2020-01-26",
  tags: ["song","live","ranking"],
  groups: ["saku","hina"],
  link: "https://docs.google.com/spreadsheets/d/1Qgkqi3t5ZX9cyOyBV7wXfnwNYR3fk40E1zj0O8il7RY/edit?gid=0#gid=0"
},
{
  name: "⭐SBA Archive",
  desc: "archive.sakamichi.co",
  detail: "坂道系畢業生部落格存檔。",
  tags: ["blog","photo","公式"],
  groups: ["nogi","saku","hina"],
  link: "https://archive.sakamichi.co/members"
},
{
  name: "🪦畢業生部落格存檔（個人）",
  desc: "歡迎提供其他網站",
  detail: "・[橋本奈々未～山下美月](https://janelin612.github.io/n46-crawler/)\n・[岩瀬佑美子～寺田蘭世](https://re-fort.net/NgzkBlogArchive/#/)\n・[今泉佑唯～渡辺梨加](https://re-fort.net/KykzkBlogArchive/#/)\n・[柿崎芽実・井口眞緒](https://re-fort.net/HntzkBlogArchive/#/)\n・個人\n　・[橋本奈々未](https://nondanee.github.io/onemoretime/nanami.hashimoto/)\n　・[久保史緒里](https://washioshihokubo.github.io/kubo-archive/)",
  tags: ["blog","photo","公式"],
  groups: ["nogi","saku","hina"],
  link: ""
},
{
  name: "⭐46◢ Online Meet Waiting Room",
  desc: "status.meet.oshi-katsu.app",
  detail: "線上見面會實時等待人數",
  tags: ["公式","meet"],
  groups: ["nogi","saku","hina"],
  link: "https://status.meet.oshi-katsu.app/"
},
{
  name: "⭐定点カメラお知らせbot",
  desc: "x.com",
  detail: "線上見面會的定點相機，成員出場自動檢測通知bot📢\n\n・[乃木坂46](https://x.com/n46_teiten)\n・[櫻坂46](https://x.com/s46_teiten)\n・[日向坂46](https://x.com/h46_teiten)",
  tags: ["streaming","tool"],
  groups: ["nogi","saku","hina"],
  link: "##"
},
{
  name: "⭐My選抜シミュレータ",
  desc: "n46sim.com",
  detail: "坂道系成員站位生成網站。\n\n・[乃木坂46](https://n46sim.com/)\n・[櫻坂46](https://sakura.n46sim.com/)\n・[日向坂46](https://hinata.n46sim.com/)",
  tags: ["create","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://n46sim.com/"
},
{
  name: "⭐坂道ソート",
  desc: "sakamichi-sort.com",
  detail: "乃木坂46、櫻坂46、日向坂46成員個人排行測驗。",
  tags: ["create","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://sakamichi-sort.com/"
},
{
  name: "⭐N46Sorter",
  desc: "n46sorter.com",
  detail: "支援乃木坂46現役及畢業成員。透過二選一建立你的推し排名，還能重現選拔陣型。結果可保存為高畫質分享圖片。專為乃木坂粉絲打造的推しメン排序工具。",
  tags: ["create","oshi"],
  groups: ["nogi"],
  link: "https://n46sorter.com/ja"
},
{
  name: "⭐Repo手帳",
  desc: "repo.zakazaka.top",
  detail: "生成見面會REPO圖",
  tags: ["create","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://www.repo.zakazaka.top/"
},
{
  name: "MSG生成器",
  desc: "msg.sakamichi-tools.cn",
  detail: "成員MSG樣式產生器，一鍵產生官方訊息格式圖片",
  tags: ["message","create"],
  groups: ["nogi","saku","hina"],
  link: "https://msg.sakamichi-tools.cn/"
},
{
  name: "⭐生寫生成器",
  desc: "photocard.sakamichi-tools.cn",
  detail: "可製作生寫樣式的圖片。",
  tags: ["photo","create","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://photocard.sakamichi-tools.cn/"
},
{
  name: "⭐App「坂道ハウス」",
  desc: "hinata-house-prod.web.app",
  detail: "僅有iOS版本，安卓版本已不再維護。可實時推送官方新聞與部落格消息，查看まとめ與成員個人資訊，SHOWROOM開播推送、SNS消息與網路新聞統合等功能。\n\n【iOS】\n・[乃木ハウス](https://apps.apple.com/jp/app/%E4%B9%83%E6%9C%A8%E3%83%8F%E3%82%A6%E3%82%B9-for-%E4%B9%83%E6%9C%A8%E5%9D%8246/id776867932)\n・[櫻ハウス](https://apps.apple.com/jp/app/%E6%AB%BB%E3%83%8F%E3%82%A6%E3%82%B9-for-%E6%AB%BB%E5%9D%8246/id1097369736)\n・[ひなたハウス](https://apps.apple.com/jp/app/%E3%81%B2%E3%81%AA%E3%81%9F%E3%83%8F%E3%82%A6%E3%82%B9-for-%E6%97%A5%E5%90%91%E5%9D%8246/id1457577580)",
  tags: ["blog","information","photo","forum","tool","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://hinata-house-prod.web.app/"
},
{
  name: "⭐App「Otaac」",
  desc: "otaac.me",
  detail: "由社群維護的偶像日程管理App。",
  tags: ["tool","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://www.otaac.me/service/app"
},
{
  name: "⭐坂道46まとめ",
  desc: "部落格相關工具",
  detail: "App中包含最新新聞與部落格更新及まとめ統整。\n\n・[iOS](https://apps.apple.com/jp/app/%E5%9D%82%E9%81%9346%E3%81%BE%E3%81%A8%E3%82%81/id1508350322)・[Android](https://play.google.com/store/apps/details?id=stephennn.blogdownload)\n・[Telegram Bot](https://t.me/NogiBlog_bot)：Showroom開播通知會付m3u8連結\n・Telegram 個別頻道\n　・[乃木坂46ブログ](https://t.me/s/nogizaka_blog) 2022-03-02~\n　・[櫻坂46ブログ](https://t.me/s/keyakizaka46blog) 2020-07-08~\n　・[日向坂46ブログ](https://t.me/s/hinatazakablog) 2021-02-23~",
  tags: ["公式","sns","blog","streaming","photo","tool","oshi"],
  groups: ["nogi","saku","hina"],
  link: "https://x.com/stephen64860726/status/1679410636902785024"
},
{
  name: "部落格通知／内容存檔",
  desc: "Telegram 頻道",
  detail: "・[乃木坂46 ブログ](https://t.me/s/Nogizaka46_blog) 2024-12-27~\n・[櫻坂46 ブログ](https://t.me/s/Sakurazaka46_Blog) 2025-10-10~\n・[日向坂46 ブログ](https://t.me/s/Hinatazaka46_Blog) 2025-10-09~",
  tags: ["公式","blog","photo"],
  groups: ["nogi","saku","hina"],
  link: ""
},
{
  name: "⭐坂道工具箱",
  desc: "[擴充功能] greasyfork.org",
  detail: "・快速跳轉博客翻譯網站\n・官網資源獲取\n・提供櫻坂與日向坂官網快速回到頂部的功能\n・解除官網的限制\n・網頁懶加載",
  tags: ["tool"],
  groups: ["nogi","saku","hina"],
  link: "https://greasyfork.org/zh-TW/scripts/565367-%E5%9D%82%E9%81%93%E5%B7%A5%E5%85%B7%E7%AE%B1"
},
{
  name: "⭐米醬圖片一鍵下載",
  desc: "[擴充功能] chromewebstore.google.com",
  detail: "一鍵批量下載當前新聞網頁的相關圖片（最大解析度）。",
  tags: ["tool"],
  groups: [],
  link: "https://chromewebstore.google.com/detail/modelpress-image-download/nepeemhbchedjbebdekhpkhknljppmif"
},
{
  name: "Rajiko",
  desc: "[擴充功能] chromewebstore.google.com",
  detail: "Rajiko 是一款可以解除 radiko.jp 地理位置限制的工具！享受來自大阪、長崎，甚至東京、青森等海外地區的廣播節目！\n解鎖 radiko.jp 的地理位置限制，收聽來自日本各地以及日本國內和海外的廣播電台。",
  tags: ["tool"],
  groups: [],
  link: "https://chromewebstore.google.com/detail/rajiko/ejcfdikabeebbgbopoagpabbdokepnff"
},
{
  name: "TVAL now",
  desc: "tval-now.switch-m.com",
  detail: "日本電視台實時收視率",
  tags: ["tool","ranking","公式"],
  groups: [],
  link: "https://tval-now.switch-m.com/"
},
{
  name: "番組表.Gガイド",
  desc: "bangumi.org",
  detail: "・熱度排行\n・今日節目表\n　・[地上波](https://bangumi.org/epg/td)\n　・[BS](https://bangumi.org/epg/bs)\n　・[CS](https://bangumi.org/epg/cs)\n　・[ラジオ](https://bangumi.org/epg/radio)",
  tags: ["ranking","公式"],
  groups: [],
  link: "https://bangumi.org/"
},
{
  name: "電視出演時長排行",
  desc: "tv-ranking.com",
  detail: "本網站根據電視節目指南中列出的藝人信息，發布藝人出場時間和節目數量的排名。由於數據是根據電視節目指南計算得出，因此並不反映節目的受歡迎程度（收視率）。即使藝人出現在節目中，如果未在電視節目指南中列出，則不會被計入排名。此外，即使藝人出現，其在節目中的出場時長也無法確定。在本網站中，「電視節目指南中列出的藝人」指的是「對節目推廣有價值」的藝人。我們希望您能將此視為衡量節目製作方對藝人價值的一種方式。",
  tags: ["ranking","公式"],
  groups: [],
  link: "https://www.tv-ranking.com/"
},
{
  name: "⭐日本廣播播放器",
  desc: "radio.nogi46.me",
  detail: "・無限制實時廣播收聽\n・存檔回放功能",
  tags: ["公式","tool"],
  groups: ["nogi","saku","hina"],
  link: "https://radio.nogi46.me/"
},
{
  name: "有名人Instagram排行網",
  desc: "insta.refetter.com",
  detail: "可檢索並顯示Instagram帳號過往追蹤人數推移",
  tags: ["sns"],
  groups: ["公式"],
  link: "https://insta.refetter.com/"
},
{
  name: "乃木坂46 卒業メンバー SNS",
  desc: "Telegram 頻道",
  detail: "元·乃木坂46的SNS，與關聯新聞的頻道",
  tags: ["公式","sns","information","photo"],
  groups: [],
  link: "https://t.me/nogizaka46_grad_SNS"
},
{
  name: "COSTUME | 櫻坂46公式サイト",
  desc: "sakurazaka46.com",
  detail: "櫻坂46公式衣裝",
  tags: ["clothes","公式"],
  groups: ["saku"],
  link: "https://sakurazaka46.com/s/s46/page/costume?ima=0000"
},
{
  name: "東京パソコンクラブ",
  desc: "unityroom.com",
  detail: "節目內做的小遊戲存檔",
  tags: ["公式","game"],
  groups: ["nogi"],
  link: "https://unityroom.com/users/cnt54a3dio78skjxv9fe"
},
{
  name: "FM-FUJI \"STUDIO ViViD\"",
  desc: "twitcasting.tv",
  detail: "FM FUJI 廣播官方直播房間（無公開過往直播存檔）",
  tags: ["公式","streaming"],
  groups: ["nogi"],
  link: "https://twitcasting.tv/c:fmfuji_twitcasting/"
},
{
  name: "字幕匯出工具",
  desc: "photocard.sakamichi-tools.cn",
  detail: "・格式轉換: 支援 JSON (CapCut/Whisper), SRT, ASS, TXT 多種格式的轉換。\n・字幕合併: 智慧合併時間軸相近的短句，優化斷句，尤其針對日語語氣詞和短句進行了優化。\n・文本處理: 提供大量刪除日語語氣詞、文字替換等功能。\n・[SRT/ASS 時間軸修復工具](https://photocard.sakamichi-tools.cn/srt_fixer.html)\n・[字幕FAD特效工具](https://photocard.sakamichi-tools.cn/fad_tool.html)",
  tags: ["tool"],
  groups: [],
  link: "https://photocard.sakamichi-tools.cn/subtitle.html"
},
{
  name: "歌曲檢定",
  desc: "music-kentei.net",
  detail: "歌曲熟悉度測驗",
  tags: ["oshi"],
  groups: [],
  link: "https://music-kentei.net/"
},
];

var tagLabels = {
  "item": "單品・愛用品",
  "clothes": "私服・衣服・制服",
  "place": "地點・聖地巡禮",
  "sns": "SNS",
 "blog": "Blog",
  "meet": "握手・線握",
  "information": "個人・團體資料",
  "song": "歌曲資料",
  "sale": "銷量",
 "game": "遊戲", 
  "streaming": "直播・串流",
  "photo": "照片・生寫",
  "message": "Message",
  "create": "生成工具",
  "oshi": "推活相關", 
  "ranking": "排行", 
  "forum": "論壇・RSS・彙總",
  "live": "演唱會・歌單",
  "tool": "相關工具",
};
